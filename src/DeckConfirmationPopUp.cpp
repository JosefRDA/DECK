...
void DeckConfirmationPopUp::render(void) {
    ...
    this->_oled.setCursor(4,34);
    if(this->isOkSelected()) {
        this->_oled.setTextColor(BLACK, WHITE);
    }
    this->_oled.print(this->_okLabel.substring(0,3));
    this->_oled.setTextColor(WHITE, BLACK);
    this->_oled.print("   ");
    if(!this->isOkSelected()) {
        this->_oled.setTextColor(BLACK, WHITE);
    }
    this->_oled.print(this->_koLabel.substring(0,3));
    this->_oled.setTextColor(WHITE, BLACK);
    this->_oled.display();
}
